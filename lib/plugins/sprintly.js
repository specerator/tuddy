...
class Sprintly {
  ...
  push(stories) {
    return Promise.all(stories.map((story) => {
      var a = { url: this.url(), headers: this.headers, formData: Sprintly.toSprintly(story) };
      return request.post({ url: this.url(), headers: this.headers, formData: Sprintly.toSprintly(story) }).spread((res, body) => {
        if (res.statusCode == 200) {
