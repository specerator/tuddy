...
class Teuxdeux {
   ...
   */
  _login() {
    return request.get({ url: 'https://teuxdeux.com/login' }).spread((res, body) => {
      var $ = cheerio.load(body);
      this.integration.authenticity_token = $('input[name=authenticity_token]').val();
    }).then(() => {
      return request.post({ url: 'https://teuxdeux.com/login', form: this.integration }).spread(function(res, body){
        return true;
      });
    });
  }
